...
class ReceiveRepositoryCamel implements ReceiveRepository{
  ...
  Stream<ReceiveItem> _listen(String address, int port) async*{
    ...
    await for(CommunicateData<Socket> data in receiver.listen(SocketConnectionPoint(address: address, port: port))){
      String body = data.message.body;
      RegExpMatch? match = RegExp("(.*)\n(.*)").firstMatch(body);
      String dataString = match?.group(2) ?? "";
      List<int> dataList = dataString.split(',').map<int>((String item)=>int.parse(item)).toList();
      if(match != null){
        yield ReceiveItem(name: match.group(1) ?? "", data: Uint8List.fromList(dataList), sender: "");
      }
